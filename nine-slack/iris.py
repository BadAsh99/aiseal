#!/usr/bin/env python3
"""
IRIS — Integrated Risk Insight System
...
SYSTEM_PROMPT = """You are IRIS — Integrated Risk Insight System — the AI security intelligence layer of the AISeal platform.
...
Always sign off responses with: *— IRIS*"""

PERSONAL_PROMPT = """You are IRIS — personal AI assistant to Ash Clements.
...
def cmd_daily_digest(owner: str = None) -> str:
    ...
    lines = [f"☀️ *IRIS DAILY DIGEST — {today}*", "━" * 44, ""]
    ...
    lines.append("_Use `@IRIS prep <customer>` before any call. — IRIS_")
    return "\n".join(lines)
...
• anything else — ask IRIS directly"""
...
def route_command(text: str, user_name: str) -> str | None:
    ...
    if t in ("whoami", "who am i", "my id"):
        return f"You are `{user_name}` — that's what IRIS sees as your owner ID. Use this in customers.json."
...
def ask_iris(question: str, personal: bool = False, thread_id: str = None) -> str:
...
def handle_text(text: str, user_id: str, say, personal: bool = False, thread_ts: str = None):
    ...
    if text.lower().strip() in ("/forget", "forget", "clear memory", "forget thread"):
        ...
        say(text="Memory cleared for this thread. *— IRIS*", **reply_kwargs)
        return
    ...
    reply = ask_iris(text, personal=personal, thread_id=thread_id)
...
def handle_mention(event, say):
    ...
    if not text:
        say(text="You called? Try `help` for Lumen commands or ask me anything. *— IRIS*",
            thread_ts=thread_ts)
        return
    ...
    while True:
        ...
        if now.hour == DIGEST_HOUR and now.date() != posted_today:
            try:
                ...
                print(f"[IRIS] Daily digest posted to {DIGEST_CHANNEL}", flush=True)
            except Exception as e:
                print(f"[IRIS] Digest post failed: {e}", flush=True)
        _time.sleep(60)
